...
SelectionRect.prototype.createDiv = function(id, z_index, class_name, cursor, action) {
	...
	if (this.isMSIE)
		div.innerHTML = '<img _action="' + action + '" src="js/itool/n.gif" style="width: 100%; height: 100%" />';
...
SelectionRect.prototype.setRect = function(x, y, width, height) {
	...
	if (x<0 || y<0 || x+this.min_width>this.maxX || y+this.min_height>this.maxY) {
...
SelectionRect.prototype.mouseDownHandler = function(e) {
	...
	if (typeof(mx) != "undefined" && typeof(my) != "undefined") {
		...
		this.rbX = r.x + r.width;
		this.rbY = r.y + r.height;
		...
		switch (this.action) {
			...
			default:
				mx = Math.min(this.maxX - this.min_width, mx);
				my = Math.min(this.maxY - this.min_height, my);
				this.mouseDownX = mx;
				this.mouseDownY = my;
...
SelectionRect.prototype.mouseMoveHandler = function(e) {
	...
	if (typeof(mx) != "undefined" && typeof(my) != "undefined") {
		...
		mdy = this.mouseDownY;
		if (this.fixedProps) {
			var k = this.min_width / this.min_height;
			switch (this.action) {
				case "nw-resize":
					mx = Math.min(this.rbX - this.min_width, mx);
                    mx = Math.max(mx, Math.floor(this.rbX-this.rbY*k));
                    my = this.rbY-Math.floor((this.rbX-mx)/k);
					this.setRect(mx, my, this.rbX - mx, this.rbY - my);
					break;



				case "ne-resize":
					mx = Math.min(this.maxX, Math.max(mx, this.x + this.min_width));
                    mx = Math.min(mx, Math.floor( this.rbY*k + this.x ));
					my = this.rbY-Math.floor((mx-this.x)/k);
					this.setRect(this.x, my, mx - this.x, Math.floor((mx-this.x)/k));
					break;

				case "sw-resize":
					mx = Math.min(this.rbX - this.min_width, mx);
					mx = Math.max(mx, Math.floor(this.rbX-(this.maxY - this.y)*k));
					this.setRect(mx, this.y, this.rbX-mx, Math.floor((this.rbX-mx)/k));
					break;

				case "se-resize":
					mx = Math.min(Math.min(this.maxX, Math.floor((this.maxY-this.y) * k + this.x)), mx);
                    h = Math.floor((mx-this.x)/k);
					this.setRect(this.x, this.y, mx - this.x, h);
					break;

				case "move":
					mx = Math.min(this.maxX - this.mouseDownRect.width - this.panDX, mx);
					my = Math.min(this.maxY - this.mouseDownRect.height - this.panDY, my);
				...
				default:
					my = this.y + Math.floor((mx-this.x)/k);
			...
			switch (this.action) {
				case "nw-resize":
					my = Math.min(this.rbY - this.min_height, my);
					mx = Math.min(this.rbX - this.min_width, mx);
...
SelectionRect.prototype.keyDownHandler = function(e) {
	this.fixedProps =true; //e.shiftKey;
}

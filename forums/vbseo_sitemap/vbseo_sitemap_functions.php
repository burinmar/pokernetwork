...
// ================================================================================
	...
	function vbseo_url_cms($drow, $old = false)
	{
		...
		{
			$cmspref = $vboptions['site_tab_url'];
			if($vboptions['friendlyurl'] == 3) $cmspref = str_replace('.'.VBSEO_PHP_EXT, '/', $cmspref);
			$url = 
			$vbseo_vars['isvb4'] ?
				preg_replace('#((vbseo.*?|cron.*?)(\.'.VBSEO_PHP_EXT.'|/))#', $cmspref, fetch_seo_url('vbcms|nosession', $drow))

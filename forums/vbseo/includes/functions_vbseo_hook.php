...
function vbseo_prepare_int_replace($ptext)
{
	...
	if (VBSEO_REWRITE_THREADS_ADDTITLE)
	{
		...
		{
			$ptext = preg_replace_callback('#(href=")(' . $matchfull . VBSEO_FIND_P_FORMAT . '[^/"]*")#is', function($m) {
				return vbseo_replace_urls_mini_post($m[1], $m[2], $m[3]);
			}, $ptext);
			$ptext = preg_replace_callback('#(href=")(' . $matchfull . 'showpost\.' . VBSEO_VB_EXT . '\?[^"]*?p(?:ostid)?=(\d+)[^/"]*")#is', function($m) {
				return vbseo_replace_urls_mini_post($m[1], $m[2], $m[3]);
			}, $ptext);
		}
		$ptext = preg_replace_callback('#(href=")(' . $matchfull .  VBSEO_FIND_MT_FORMAT . '[^/"]*")#is', function($m) {
			return vbseo_replace_urls_mini($m[1],$m[2],$m[3],'m');
		}, $ptext);
		$ptext = preg_replace_callback('#(href=")(' . $matchfull .  VBSEO_FIND_T_FORMAT . '[^/"]*")#is', function($m) {
			return vbseo_replace_urls_mini($m[1],$m[2],$m[3]);
		}, $ptext);
		$ptext = preg_replace_callback('#(href=")(' . $matchfull . '(?:show|print)thread\.' . VBSEO_VB_EXT . '\?[^"]*?t(?:hreadid)?=(\d+)[^/"]*")#is', function($m) {
			return vbseo_replace_urls_mini($m[1],$m[2],$m[3]);
		}, $ptext);
	}
	return $ptext;
...
$pmsg = preg_replace_callback('#' . preg_quote($redurl, '#') . '([^"\]\[]*)#is', function($m){

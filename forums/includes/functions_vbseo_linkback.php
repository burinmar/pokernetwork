...
$rq = $db->vbseodb_query_first($q = "SELECT t_id as cnt FROM " . vbseo_tbl_prefix('vbseo_linkback') . "
...
. ( ($approve && !$wait) ? 1 : 0) . ", \"$incoming\"," . ($wait?1:0) .")"
);
if ($incoming)
{
...
(preg_match('#' . VBSEO_PINGBACK_STOPWORDS . '#', $snippet) ||
preg_match('#' . VBSEO_PINGBACK_STOPWORDS . '#', $title) ||
preg_match('#' . VBSEO_PINGBACK_STOPWORDS . '#', $threadinfo['title'])
))

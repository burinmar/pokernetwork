...
class special extends moon_com {
	...
	function viewList($vars) {
		...
		//$m['site-slave'] = _SITE_ID_ == 'com' ? 0 : 1;
		$m['site-slave'] = 1;
	...
	function saveItem()
	{
		...
		//save to database
		$ins = $form->get_values('date', 'name', 'qualification_from', 'qualification_to', 'qualification_points', 'timezone', 'exclusive', 'prizepool', 'room_id', 'team_id', 'body', 'tags', 'hide', 'password', 'password_from');
		$locale = & moon :: locale();
	...
	function getRooms($id = FALSE)
	{
		...
		$dat = $this->db->array_query_assoc($sql . ' ORDER BY name');
		$m = array();
		foreach ($dat as $v) {
			$m[$v['id']] = $v;
		}

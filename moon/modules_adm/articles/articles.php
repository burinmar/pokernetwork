...
function getImageSrc($imageFn, $prefix = '')
{
	...
	if ($imagePrefix != NULL && $imagePrefix != _SITE_ID_) {
...
function getTreeUriConstruct($items, $currentParent, &$tree, $currLevel = 0, $prevLevel = -1)
{
	foreach ($items as $categoryId => $category) {
		if ($currentParent == $category['parent_id']) {
			...
			//	$uri = ' -- ' . $uri;
			//}
			$parentUri = !empty($tree[$category['parent_id']]) ? $tree[$category['parent_id']] : '';
			$tree[$categoryId] = $parentUri ? $parentUri . '/' . $uri : $uri;

			if ($currLevel > $prevLevel) {
				$prevLevel = $currLevel;
			}
			$currLevel++;
		 	$this->getTreeUriConstruct($items, $categoryId, $tree, $currLevel, $prevLevel);
		 	$currLevel--;
		}
	}
	return $tree;
